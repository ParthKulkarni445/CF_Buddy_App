...
authRouter.post("/api/signup", async (req, res) => {
  try {
    ...
    if (cf.data.status !== 'OK') {
      return res.status(400).send({ msg: 'Invalid Codeforces handle' });
    }
...
authRouter.delete("/api/delete-account", auth, async (req, res) => {
  try {
    const userId = req.user; // set by tokenverify middleware
    ...
    return res.status(500).json({
      success: false,
      msg: "An error occurred while deleting the account. Please try again later."
    });
  }
});
